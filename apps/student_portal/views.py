...
Demerit = None  # no separate demerit model — likely folded into DisciplineIncident
...
def dashboard(request):
    ...
    # Attendance
    attendance_qs = AttendanceRecord.objects.filter(student=student, term=term) if term else []
    ...
    if term:
        ...
        today_timetable = TimetableEntry.objects.filter(
            class_group=student.current_class,
            day_of_week=timezone.now().weekday(),   # 0=Mon
    ...
    recent_results = ExamResult.objects.filter(
        student=student
    ).select_related('subject', 'term').order_by('-term__end_date')[:5]
...
def assignments(request):
    ...
    if filter_tab == 'pending':
        qs = base_qs.exclude(
            submissions__student=student, submissions__is_submitted=True
        )
...
def assignment_detail(request, assignment_id):
    student    = request.student
    assignment = get_object_or_404(Assignment, id=assignment_id, class_group=student.current_class)
    ...
    if request.method == 'POST' and not submission:
        ...
        if uploaded_file:
            # Upload to Cloudinary (Phase 1 utility)
            from utils.cloudinary import upload_file
            sub.file_path = upload_file(uploaded_file, folder='submissions')
            sub.save(update_fields=['file_path'])
...
def quizzes(request):
    ...
    all_quizzes = Quiz.objects.filter(
        class_group=student.current_class, is_published=True
...
def results(request):
    ...
    results_qs = ExamResult.objects.filter(
        student=student
    ).select_related('subject', 'term').order_by('-term__end_date', 'subject__name')

    if term_filter:
        results_qs = results_qs.filter(term_id=term_filter)
...
def transcript(request):
    ...
    all_results = ExamResult.objects.filter(
        student=student
    ).select_related('subject', 'term', 'term__academic_year').order_by(
        'term__academic_year__start_date', 'term__start_date', 'subject__name'
    )
...
def fee_account(request):
    ...
    payments   = Payment.objects.filter(invoice__student=student, status='success').order_by('-created_at')

    total_billed = invoices.aggregate(s=Sum('total'))['s'] or 0
...
def library(request):
    ...
    ).select_related('book').order_by('-returned_at')[:20]

    query     = request.GET.get('q', '')
    catalogue = Book.objects.filter(is_available=True)
    if query:
...
def merits(request):
    ...
    merit_records  = Merit.objects.filter(student=student).order_by('-awarded_on')
    demerit_records = Demerit.objects.filter(student=student).order_by('-awarded_on')
    incidents       = DisciplineIncident.objects.filter(student=student).order_by('-date')[:10]

    if term:
        merit_records   = merit_records.filter(term=term)
        demerit_records = demerit_records.filter(term=term)

    merit_total   = merit_records.aggregate(s=Sum('points'))['s'] or 0
...
def academic_calendar(request):
    ...
    events = AcademicCalendarEvent.objects.filter(
        ...
        Q(audience='all') |
        Q(audience=request.tenant.institution_type) if hasattr(request, 'tenant') else Q()
    ).order_by('start_date')
...
def _enrollment_checklist(student, term):
    ...
    if invoice and invoice.total > 0:
        ...
        fee_pct = (paid / invoice.total) * 100
...
def mark_notification_read(request, notif_id):
    notif = get_object_or_404(PortalNotification, id=notif_id, student=request.user.student)
    notif.mark_read()
...
def notifications_feed(request):
    ...
    notifs   = PortalNotification.objects.filter(student=student).order_by('-created_at')[:20]
